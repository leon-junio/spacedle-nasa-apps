...
public class App {
    ...
    public static boolean setFilesDir(String dir) {
        try {
            files = new ArrayList<>(Arrays.asList(new File("resources/" + dir + "/pics/").listFiles()));
            infos = new ArrayList<>(Arrays.asList(new File("resources/" + dir + "/desc/").listFiles()));
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public static boolean taskFotos(int op) throws IOException {
        ...
        try {
            try {
                if (files.isEmpty()) {
                    files = new ArrayList<>(Arrays.asList(new File("resources/" + getType(op)).listFiles()));
                }
