...
public class App {
    ...
    public static boolean setFilesDir(String dir) {
        try {
            files = new ArrayList<>(Arrays.asList(new File("classes/resources/" + dir + "/pics/").listFiles()));
            infos = new ArrayList<>(Arrays.asList(new File("classes/resources/" + dir + "/desc/").listFiles()));
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public static boolean taskFotos(int op) throws IOException {
        ...
        try {
            try {
                if (files.isEmpty()) {
                    files = new ArrayList<>(Arrays.asList(new File("classes/resources/" + getType(op)).listFiles()));
                }
