...
const Universal = ({ enteringTab }) => {
  const [trys, setTrys] = useState(0);
  const [lives, setLives] = useState(5);
  const [localDataLoaded, setLocalDataLoaded] = useState(false);

  const { data: imageData, loading: loadingImageData, error: errorImageData } = useImage(trys, 2);
  const { data: namesData, loading: loadingNamesData, error: errorNamesData } = useNames(2);
  ...
  useEffect(() => {
    if (enteringTab && !localDataLoaded) {
      const localData = localStorage.getItem("universal");
      if (localData) {
  ...
  useEffect(() => {
    if (localDataLoaded) {
      ...
      localStorage.setItem("universal", JSON.stringify(dataToStorage));
    }
  }, [win, lives, trys]);
